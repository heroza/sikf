<?php
class Auditee_model extends CI_Model
	{
		...
		function auditeeWithId($id_auditee)
		{
			$result=$this->db->query("SELECT * FROM auditee WHERE id_auditee = $id_auditee");
			return $result;
		}

		function auditeeWithProdi($prodi)
		{
			$query=$this->db->query("SELECT * FROM auditee WHERE prodi = '$prodi'");
		...
		function Pilih_Laporan_Prodi($id2)
		{
			$query=$this->db->query("select jadwal.id_jadwal, auditee.prodi, instrumen.instrumen, jadwal.validasi_auditee, jadwal.validasi_auditor from jadwal, auditee, instrumen where auditee.prodi = '$id2' AND  jadwal.id_auditee = auditee.id_auditee AND instrumen.id_instrumen = jadwal.id_instrumen");
			return $query;
		}
		...
		{
			$query=$this->db->query("select jadwal.id_jadwal, auditee.prodi, instrumen.instrumen, jadwal.validasi_auditee, jadwal.validasi_auditor from jadwal, auditee, instrumen where jadwal.id_periode = $id AND auditee.prodi = '$id2' AND  jadwal.id_auditee = auditee.id_auditee AND instrumen.id_instrumen = jadwal.id_instrumen");
			return $query;
		}
		...
		function total_penelitian_group($id)
		{
			$query=$this->db->query("select tahun_pelaksanaan, count(kode) as jumlah from dosen_meneliti, dosen, prodi, penelitian where dosen.id_prodi = prodi.id_prodi AND prodi = '$id' AND kode_meneliti_sebagai = 3 AND dosen_meneliti.kode_dosen = dosen.kode_dosen AND penelitian.kode_penelitian = dosen_meneliti.kode_penelitian GROUP BY tahun_pelaksanaan");
			return $query;
		}
	}
?>