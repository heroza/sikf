...
$data8[] = $sum_standar;	// menyimpan nilai total rubrik .
}
...
<?php 
	$i = 1;
	$data['jadwal_auditor'] = $this->Pimpinan_model->Total_Data("jadwal_auditor, auditor where auditor.id_auditor = jadwal_auditor.id_auditor AND id_jadwal = $id_jadwal AND validasi_auditor = 'Sudah'");
	foreach($data['jadwal_auditor']->result_array() as $ja)
	{ 
		echo $i.". ".$ja['nama']." : "; 
		$i++;
		$id_jadwal_auditor = $ja['id_jadwal_auditor'];
		$data['kunjungan_auditor'] = $this->Pimpinan_model->Total_Data("kunjungan_auditor where id_jadwal_auditor = $id_jadwal_auditor");
