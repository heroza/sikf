<section id="page">
				<aside id="sidebar">
					<div id="logo">
						<a><img src="<?php echo base_url(); ?>asset/images/logo.png"/></a>
					</div>
					<div class="menus">
						<!-- menu #1 -->
						<nav>
							<h2>Main Menu</h2>
							
							<ul>
								<li>
									<a href="<?php echo base_url(); ?>pimpinan/">
										<i class="glyph-cloud"></i>
										<div class="label">Dashboard</div>
									</a>
								</li>
								<li>
									<a href="<?php echo base_url(); ?>pimpinan/prodi">
										<i class="glyph-portrait-view"></i>
										<div class="label">Prodi</div>
									</a>
								</li>
								<li>
									<a href="<?php echo base_url(); ?>pimpinan/matkul">
										<i class="glyph-portrait-view"></i>
										<div class="label">Mata Kuliah</div>
									</a>
								</li>
								<li>
									<a href="<?php echo base_url(); ?>pimpinan/dosen">
										<i class="glyph-portrait-view"></i>
										<div class="label">Dosen</div>
									</a>
								</li>
								<li>
									<a href="<?php echo base_url(); ?>pimpinan/penelitian">
										<i class="glyph-portrait-view"></i>
										<div class="label">Penelitian</div>
									</a>
								</li>
                                <li>
									<a href="<?php echo base_url(); ?>pimpinan/publikasi">
										<i class="glyph-portrait-view"></i>
										<div class="label">Publikasi</div>
									</a>
								</li>
                                <li>
									<a href="<?php echo base_url(); ?>pimpinan/pm">
										<i class="glyph-portrait-view"></i>
										<div class="label">Pengabdian Masyarakat</div>
									</a>
								</li>
                                <li>
									<a href="<?php echo base_url(); ?>pimpinan/change_password">
										<i class="glyph-portrait-view"></i>
										<div class="label">Ubah Password</div>
									</a>
								</li>
                        	</ul>
						</nav>
						<!-- /menu -->
					</div>
				</aside>