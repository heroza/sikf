...
$data['strategi_bsc'] = $this->Auditor_model->Strategi_Bsc("$id_instrumen"); 
foreach($data['strategi_bsc']->result_array() as $str_bsc)
{
  ...
  foreach($perspektif_bsc->result_array() as $p_bsc)
  {
	...
	$data['standar_rubrik_bsc'] = $this->Auditor_model->Standar_Rubrik_Bsc("$id_instrumen","$id_perspektif","$id_strategi"); 
...
?>
    ...
    <table width="100%">
				...
				{   $id_auditor = $ja['id_auditor'];
					$data['auditor'] = $this->Auditor_model->Total_Data("auditor where auditor.id_auditor = $id_auditor");
...
<?php 
	$i = 1;
	$data['jadwal_auditor'] = $this->Auditor_model->Total_Data("jadwal_auditor, auditor where auditor.id_auditor = jadwal_auditor.id_auditor AND id_jadwal = $id_jadwal AND validasi_auditor = 'Sudah'");
	foreach($data['jadwal_auditor']->result_array() as $ja)
	{ 
		echo $i.". ".$ja['nama']." : "; 
		$i++;
		$id_jadwal_auditor = $ja['id_jadwal_auditor'];
		$data['kunjungan_auditor'] = $this->Auditor_model->Total_Data("kunjungan_auditor where id_jadwal_auditor = $id_jadwal_auditor");
